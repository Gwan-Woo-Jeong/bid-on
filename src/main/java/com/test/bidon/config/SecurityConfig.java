...
public class SecurityConfig {
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		...
		// 허가 URL
		http.authorizeHttpRequests(auth -> auth.requestMatchers("/", "/login/**", "/oauth2/**", "/join", "/joinok")
				.permitAll().requestMatchers("/my").hasAnyRole("MEMBER", "ADMIN")
		...
		// 커스텀 로그인 설정
		http.formLogin(auth -> auth.loginPage("/login") // 사용자 로그인 페이지 URL
				.defaultSuccessUrl("/").loginProcessingUrl("/loginok").permitAll());
		...
		// 로그아웃 설정을 여기로 통합
		http.logout(auth -> auth.logoutUrl("/logout").logoutSuccessUrl("/").invalidateHttpSession(true) // 세션 무효화
